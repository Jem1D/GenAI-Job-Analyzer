...
if "skill_match" not in st.session_state:
    st.session_state.skill_match = None
...
if submit and url_input:
    try:
        with st.spinner("Scraping and analyzing..."):
            ...
            # Persist analysis so Generate button works across reruns.
...
if st.session_state.job_data and st.session_state.skill_match:
    ...
    if st.button("Generate Cold Email"):
        try:
            with st.spinner("Generating cold email..."):
                ...
                st.session_state.generated_email = chain.write_mail(job_data, links, match["your_skills"])
        except Exception:
